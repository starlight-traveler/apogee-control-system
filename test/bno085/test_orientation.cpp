...
static void TestVectorTransformationMapsXAxisToNegativeZ() {
    float outX = 0.0f;
    float outY = 0.0f;
    float outZ = 0.0f;
    ...
    TEST_ASSERT_FLOAT_WITHIN(1e-6f, -1.0f, outZ);
}

static void TestVectorTransformationMapsGravityToPositiveX() {
    float outX = 0.0f;
    float outY = 0.0f;
    float outZ = 0.0f;
    bno085_orientation::TransformVector(0.0f, 0.0f, constants::kGravity, outX, outY, outZ);
    TEST_ASSERT_FLOAT_WITHIN(1e-6f, constants::kGravity, outX);
    TEST_ASSERT_FLOAT_WITHIN(1e-6f, 0.0f, outY);
    TEST_ASSERT_FLOAT_WITHIN(1e-6f, 0.0f, outZ);
}

static void TestQuaternionIdentityRemainsIdentity() {
...
static void TestQuaternionBasisChangeMapsXAxisRotationToNegativeZ() {
    const float rawW = 0.70710677f;
    const float rawX = 0.70710677f;
    ...
    TEST_ASSERT_FLOAT_WITHIN(1e-5f, -rawX, out[3]);
}

static void TestQuaternionBasisChangePreservesNorm() {
    const float rawW = 0.70710677f;
...
void setup() {
    UNITY_BEGIN();
    RUN_TEST(TestVectorTransformationMapsXAxisToNegativeZ);
    RUN_TEST(TestVectorTransformationMapsGravityToPositiveX);
    RUN_TEST(TestQuaternionIdentityRemainsIdentity);
    RUN_TEST(TestQuaternionBasisChangeMapsXAxisRotationToNegativeZ);
    RUN_TEST(TestQuaternionBasisChangePreservesNorm);
    UNITY_END();
}
