...
#ifndef ACS_BUILD_ICM20948_CALIBRATION
#error "Use the icm_calibration PlatformIO environment to build this target."
#endif

namespace {

constexpr uint8_t kChipSelectPin = settings::sensors::icm20948::kChipSelectPin;
constexpr uint8_t kAccelRangeG = settings::sensors::icm20948::kCalibrationAccelRangeG;
constexpr uint16_t kGyroRangeDps = settings::sensors::icm20948::kCalibrationGyroRangeDps;
...
struct VectorStats {
    ...
    float MeanTemperature() const {
        return count > 0 ? static_cast<float>(temperatureSum / static_cast<double>(count)) : 0.0f;
    }
};
...
void PrintSettingsInsertionGuide() {
    ...
    Serial.println("  section: namespace settings::sensors::icm20948");
...
void PrintHelp() {
    Serial.println();
    Serial.println("ICM-20948 advanced calibration");
...
bool ConfigureSensor() {
    ...
    fullScale.a = AccelFullScaleEnum(kAccelRangeG);
    fullScale.g = GyroFullScaleEnum(kGyroRangeDps);
...
void PrintCurrentSample() {
    const float ax = static_cast<float>(g_icm.agmt.acc.axes.x);
    const float ay = static_cast<float>(g_icm.agmt.acc.axes.y);
    const float az = static_cast<float>(g_icm.agmt.acc.axes.z);
    const float gx = static_cast<float>(g_icm.agmt.gyr.axes.x);
    const float gy = static_cast<float>(g_icm.agmt.gyr.axes.y);
    const float gz = static_cast<float>(g_icm.agmt.gyr.axes.z);
    const float mx = static_cast<float>(g_icm.agmt.mag.axes.x);
    const float my = static_cast<float>(g_icm.agmt.mag.axes.y);
    const float mz = static_cast<float>(g_icm.agmt.mag.axes.z);
    ...
    Serial.print(" temp_c=");
    Serial.println(g_icm.temp(), 2);
}

void CaptureAccelFace(AccelFaceIndex face) {
    g_accelFaces[face].valid = true;
    g_accelFaces[face].mean[0] = static_cast<float>(g_icm.agmt.acc.axes.x);
    g_accelFaces[face].mean[1] = static_cast<float>(g_icm.agmt.acc.axes.y);
    g_accelFaces[face].mean[2] = static_cast<float>(g_icm.agmt.acc.axes.z);
    g_accelFaces[face].temperatureC = g_icm.temp();
...
void StoreGyroTemperaturePoint() {
    if (!g_gyroCalibrationStats.Ready()) {
        return;
    }
...
void UpdateGyroCalibration(float gx, float gy, float gz, float temperatureC) {
    if (!g_gyroCalibrationActive) {
        return;
    }
    g_gyroCalibrationStats.Add(gx, gy, gz, temperatureC);
...
void UpdateMagCapture(float mx, float my, float mz) {
    g_magLast[0] = mx;
    g_magLast[1] = my;
    g_magLast[2] = mz;
    if (!g_magCaptureActive) {
    ...
    if (!g_magCaptureReady) {
        g_magMin[0] = g_magMax[0] = mx;
        g_magMin[1] = g_magMax[1] = my;
        g_magMin[2] = g_magMax[2] = mz;
        g_magCaptureReady = true;
    } else {
        g_magMin[0] = min(g_magMin[0], mx);
        g_magMin[1] = min(g_magMin[1], my);
        g_magMin[2] = min(g_magMin[2], mz);
        g_magMax[0] = max(g_magMax[0], mx);
        g_magMax[1] = max(g_magMax[1], my);
        g_magMax[2] = max(g_magMax[2], mz);
    }
    ++g_magSampleCount;
    if (g_magStoredSamples < kMaxMagSamples) {
        g_magSamples[g_magStoredSamples][0] = mx;
        g_magSamples[g_magStoredSamples][1] = my;
        g_magSamples[g_magStoredSamples][2] = mz;
        ++g_magStoredSamples;
    }
}
...
void PrintDetailedDump() {
    Serial.println();
    Serial.println("==== ICM detailed calibration dump ====");
...
void PrintRecommendedSettings() {
    ...
    Serial.println();
    Serial.println("==== BEGIN paste into src/settings.h / settings::sensors::icm20948 ====");
    PrintSettingsInsertionGuide();
...
void setup() {
    ...
    Serial.println();
    Serial.println("ICM-20948 calibration target");
    Serial.print("CS pin: ");
    Serial.println(kChipSelectPin);

    if (!ConfigureSensor()) {
        Serial.println("ICM-20948 init failed. Check wiring and SPI bus.");
        while (true) {
            delay(1000);
        }
    }
...
void loop() {
    ...
    const float my = static_cast<float>(g_icm.agmt.mag.axes.y);
    const float mz = static_cast<float>(g_icm.agmt.mag.axes.z);
    const float temperatureC = g_icm.temp();

    UpdateGyroCalibration(gx, gy, gz, temperatureC);
    UpdateMagCapture(mx, my, mz);
