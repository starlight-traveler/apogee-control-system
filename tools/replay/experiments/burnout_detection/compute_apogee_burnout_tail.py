...
OUTPUT_CSV = DATA_DIR / "output_burnout_tail_prediction.csv"
OUTPUT_COMPARE_SVG = PLOTS_DIR / "burnout_tail_comparison.svg"
OUTPUT_CONTROL_SVG = PLOTS_DIR / "burnout_tail_controls.svg"


# Late-burn thrust-tail experiment tuning.
# This variant keeps the current coast-only adaptive drag update, but during a
# gated late-burn window it carries a short thrust term aligned with the body
# axis before handing off to the normal coast predictor.
LATE_BURN_ENABLE_ACCEL_MPS2 = 12.0
BURNOUT_ACCEL_THRESHOLD_MPS2 = 2.0
TAIL_BODY_ACCEL_CLAMP_MPS2 = 60.0
TAIL_DECAY_TAU_S = 0.12
TAIL_STOP_BODY_ACCEL_MPS2 = 1.0
TAIL_MAX_TIME_S = 0.45
TAIL_STEP_S = 0.01
MAX_TAIL_ZENITH_DEG = 20.0
...
def _clamped_zenith_rad(zenith_rad: float) -> float:
    return _clamp(
        zenith_rad,
        -math.radians(MAX_TAIL_ZENITH_DEG),
        math.radians(MAX_TAIL_ZENITH_DEG),
    )


def _infer_initial_tail_body_accel(
    seed_state: adaptive.ApogeeState,
    force_table: adaptive.ForceTable,
    drag_scale: float,
    measured_accel_z_mps2: float,
) -> tuple[float, float]:
    predicted_accel_z_mps2, _, _, _ = adaptive._compute_accel_with_axial_scale(
        seed_state,
        force_table,
        drag_scale,
    )
    if measured_accel_z_mps2 <= BURNOUT_ACCEL_THRESHOLD_MPS2 or measured_accel_z_mps2 > LATE_BURN_ENABLE_ACCEL_MPS2:
        return 0.0, predicted_accel_z_mps2

    zenith_rad = _clamped_zenith_rad(seed_state.zenith)
    cos_zenith = max(0.25, abs(math.cos(zenith_rad)))
    excess_vertical_accel_z_mps2 = _clamp(
        measured_accel_z_mps2 - predicted_accel_z_mps2,
        0.0,
        TAIL_BODY_ACCEL_CLAMP_MPS2 * cos_zenith,
    )
    if excess_vertical_accel_z_mps2 <= 0.0:
        return 0.0, predicted_accel_z_mps2

    initial_body_accel_mps2 = _clamp(
        excess_vertical_accel_z_mps2 / cos_zenith,
        0.0,
        TAIL_BODY_ACCEL_CLAMP_MPS2,
    )
    return initial_body_accel_mps2, predicted_accel_z_mps2


def _advance_late_burn_tail(
    seed_state: adaptive.ApogeeState,
    force_table: adaptive.ForceTable,
    drag_scale: float,
    measured_accel_z_mps2: float,
) -> tuple[adaptive.ApogeeState, float, float]:
    initial_body_accel_mps2, _ = _infer_initial_tail_body_accel(
        seed_state,
        force_table,
        drag_scale,
        measured_accel_z_mps2,
    )
    if initial_body_accel_mps2 <= 0.0:
        return adaptive.ApogeeState(**seed_state.__dict__), 0.0, 0.0

    current = adaptive.ApogeeState(**seed_state.__dict__)
    elapsed_s = 0.0
    while elapsed_s < TAIL_MAX_TIME_S:
        tail_body_accel_mps2 = initial_body_accel_mps2 * math.exp(-elapsed_s / TAIL_DECAY_TAU_S)
        if tail_body_accel_mps2 < TAIL_STOP_BODY_ACCEL_MPS2:
            break

        dt_s = min(TAIL_STEP_S, TAIL_MAX_TIME_S - elapsed_s)
        if dt_s <= 0.0:
            break

        base_ax, base_ay, base_ang, _ = adaptive._compute_accel_with_axial_scale(current, force_table, drag_scale)
        zenith_rad = _clamped_zenith_rad(current.zenith)
        thrust_ax = tail_body_accel_mps2 * math.cos(zenith_rad)
        thrust_ay = tail_body_accel_mps2 * math.sin(zenith_rad)
        derivative = (
            current.vertical_v,
    ...
    return current, elapsed_s, initial_body_accel_mps2


def _predict_with_late_burn_tail(
    seed_state: adaptive.ApogeeState,
    force_table: adaptive.ForceTable,
    drag_scale: float,
    measured_accel_z_mps2: float,
) -> tuple[float, float, float]:
    tailed_state, tail_duration_s, tail_initial_body_accel_mps2 = _advance_late_burn_tail(
        seed_state,
        force_table,
        drag_scale,
        measured_accel_z_mps2,
    )
    prediction_m = adaptive._predict_apogee_with_axial_scale(tailed_state, force_table, drag_scale)
    return prediction_m, tail_duration_s, tail_initial_body_accel_mps2
...
def _write_compare_svg(rows: list[dict[str, float | str]], actual_apogee_m: float) -> None:
    ...
    burnout_tail = [float(row["burnout_tail_apogee_m"]) for row in sampled]
    ...
    t_min = min(times)
    t_max = max(times)
    y_min = min(min(altitude), min(logged), min(current), min(burnout_tail), actual_apogee_m)
    y_max = max(max(altitude), max(logged), max(current), max(burnout_tail), actual_apogee_m)
    ...
    burnout_tail_points = adaptive._polyline_points(
        times, burnout_tail, t_min, t_max, y_min, y_max, left, top, plot_w, plot_h
    )
  ...
  <text x="{width / 2:.0f}" y="36" text-anchor="middle" font-family="monospace" font-size="24">Burnout Tail Predictor Comparison</text>
  ...
  <polyline fill="none" stroke="#2ca02c" stroke-width="1.5" points="{burnout_tail_points}"/>
  ...
  <text x="{left + 100}" y="{top + 110}" font-family="monospace" font-size="16">Late-Burn Thrust-Tail Variant</text>
...
def _write_control_svg(rows: list[dict[str, float | str]]) -> None:
    ...
    tail_duration = [float(row["burnout_tail_duration_s"]) for row in sampled]
    tail_initial_body_accel = [float(row["burnout_tail_initial_body_accel_mps2"]) for row in sampled]
    ...
    tail_duration_points = adaptive._polyline_points(
        times, tail_duration, t_min, t_max, y_min, y_max, left, top, plot_w, plot_h
    )
    tail_initial_points = adaptive._polyline_points(
        times, tail_initial_body_accel, t_min, t_max, y_min, y_max, left, top, plot_w, plot_h
    )
    ...
    svg = f"""<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">
  <rect width="100%" height="100%" fill="white"/>
  <text x="{width / 2:.0f}" y="36" text-anchor="middle" font-family="monospace" font-size="24">Burnout Tail Controls</text>
  <text x="{width / 2:.0f}" y="{height - 20}" text-anchor="middle" font-family="monospace" font-size="18">Time (s)</text>
  <text x="30" y="{height / 2:.0f}" text-anchor="middle" font-family="monospace" font-size="18" transform="rotate(-90 30 {height / 2:.0f})">Scale / Accel / Tail</text>
  {''.join(grid_lines)}
  <rect x="{left}" y="{top}" width="{plot_w}" height="{plot_h}" fill="none" stroke="black" stroke-width="2"/>
  ...
  <polyline fill="none" stroke="#9467bd" stroke-width="1.4" points="{tail_duration_points}"/>
  <polyline fill="none" stroke="#ff7f0e" stroke-width="1.4" points="{tail_initial_points}"/>
  ...
  <text x="{left + 100}" y="{top + 110}" font-family="monospace" font-size="16">Tail Duration [s]</text>
  <line x1="{left + 20}" y1="{top + 132}" x2="{left + 90}" y2="{top + 132}" stroke="#ff7f0e" stroke-width="3"/>
  <text x="{left + 100}" y="{top + 138}" font-family="monospace" font-size="16">Initial Tail Body Accel</text>
</svg>
"""
...
def main() -> None:
    ...
    for active_row, merged_row in zip(active_rows, merged_rows):
        ...
        burnout_tail_apogee_m = current_logic_apogee_m
        burnout_tail_duration_s = 0.0
        burnout_tail_initial_body_accel_mps2 = 0.0
        if merged_row.status == "burn":
            burnout_tail_apogee_m, burnout_tail_duration_s, burnout_tail_initial_body_accel_mps2 = (
                _predict_with_late_burn_tail(
                    seed_state,
                    force_table,
                    drag_scale,
        ...
        output_rows.append(
            {
                ...
                "burnout_tail_apogee_m": burnout_tail_apogee_m,
                ...
                "burnout_tail_duration_s": burnout_tail_duration_s,
                "burnout_tail_initial_body_accel_mps2": burnout_tail_initial_body_accel_mps2,
    ...
    with OUTPUT_CSV.open("w", newline="") as handle:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "burnout_tail_apogee_m",
                "drag_scale",
                "predicted_accel_z_mps2",
                "measured_accel_z_mps2",
                "accel_residual_mps2",
                "axial_accel_z_mps2",
                "burnout_tail_duration_s",
                "burnout_tail_initial_body_accel_mps2",
                "status",
            ],
        )
        writer.writeheader()
    ...
    print(
        "Late-burn tail settings: "
        f"enable_accel<={LATE_BURN_ENABLE_ACCEL_MPS2:.1f}m/s^2 "
        f"burnout_accel={BURNOUT_ACCEL_THRESHOLD_MPS2:.1f}m/s^2 "
        f"tau={TAIL_DECAY_TAU_S:.2f}s max_time={TAIL_MAX_TIME_S:.2f}s"
    )
    burn_rows = [row for row in output_rows if row["status"] == "burn"]
    active_tail_rows = [row for row in burn_rows if float(row["burnout_tail_duration_s"]) > 0.0]
    if active_tail_rows:
        max_delta_row = max(
            active_tail_rows,
            key=lambda row: float(row["burnout_tail_apogee_m"]) - float(row["current_logic_apogee_m"]),
        )
        last_burn_row = burn_rows[-1]
        print(
            "burnout_tail_activation: "
            f"rows={len(active_tail_rows)} "
            f"window={float(active_tail_rows[0]['time_s']):.3f}s..{float(active_tail_rows[-1]['time_s']):.3f}s "
            f"max_delta={float(max_delta_row['burnout_tail_apogee_m']) - float(max_delta_row['current_logic_apogee_m']):.1f}m "
        ...
        print(
            ...
            f"tail={float(last_burn_row['burnout_tail_apogee_m']):.1f}m"
        )
    _print_metrics("current_logic", output_rows, "current_logic_apogee_m", actual_apogee_m)
    _print_metrics("burnout_tail", output_rows, "burnout_tail_apogee_m", actual_apogee_m)
