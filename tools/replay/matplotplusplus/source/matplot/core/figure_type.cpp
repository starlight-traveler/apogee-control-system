...
namespace matplot {
    ...
    void figure_type::send_gnuplot_draw_commands() {
        ...
        const std::size_t axes_count = children_.size();
        if (axes_count == 0) {
            include_comment("An empty plot");
            plot_empty_plot();
        } else if (axes_count == 1) {
            include_comment("Single axes plot");
            const bool should_configure_background = backend_->output().empty();
            if (should_configure_background) {
                include_comment("Window background");
                run_window_color_command();
            }
            children_.front()->run_commands();
            if (should_configure_background) {
                include_comment("Unset window background");
