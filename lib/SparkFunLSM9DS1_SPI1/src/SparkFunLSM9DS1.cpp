...
uint16_t LSM9DS1::begin(uint8_t agAddress, uint8_t mAddress, TwoWire &wirePort)
{
	...
	const gyroSettings requestedGyro = settings.gyro;
	const accelSettings requestedAccel = settings.accel;
	const magSettings requestedMag = settings.mag;
	const temperatureSettings requestedTemp = settings.temp;
	...
	init();
	settings.gyro = requestedGyro;
	settings.accel = requestedAccel;
	settings.mag = requestedMag;
	settings.temp = requestedTemp;
...
uint16_t LSM9DS1::beginSPI(uint8_t ag_CS_pin, uint8_t m_CS_pin, SPIClass &spiPort)
{
	// Set device settings, they are used in many other places
...
void LSM9DS1::SPIwriteByte(uint8_t csPin, uint8_t subAddress, uint8_t data)
{
	...
	_spiPort->transfer(subAddress & 0x3F); // Send Address
	_spiPort->transfer(data); // Send data
							...
							uint8_t * dest, uint8_t count)
{
	...
	digitalWrite(csPin, LOW); // Initiate communication
	_spiPort->transfer(rAddress);
	for (int i=0; i<count; i++)
	{
		dest[i] = _spiPort->transfer(0x00); // Read into destination array
	}
	digitalWrite(csPin, HIGH); // Close communication
